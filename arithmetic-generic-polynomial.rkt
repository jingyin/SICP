...
(define (install-complex-package)
  ...
  (put 'raise '(complex) (lambda (f) (make-polynomial-constant (tag f))))
...
(define (install-polynomial-package)
  ...
  (define (same-variable? x y)
    (and (variable? x) (variable? y)
         (or (eq? x y)
             (eq? x special-var)
             (eq? y special-var))))
  (define (select-variable x y)
    (if (eq? x special-var) y x))
  ...
  (define (add-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (select-variable (variable p1) (variable p2))
  ...
  (put 'raise '(polynomial) (lambda (p) #f))
  ...
  (put 'make-from-constant 'polynomial (lambda (term) (tag (make-poly special-var (make-dense-terms (list term))))))
  'done)
...
(define (make-polynomial-constant term)
  ((get 'make-from-constant 'polynomial) term))
...
(mul poly-dividend (make-scheme-number 3))
(add poly-dividend (make-scheme-number 3))
